...
export default function Home() {
  ...
  async function loadLeads(email = user?.email) {
    if (!email) return;
    try {
      const result = await api('listLeads', { vendedor: email });
      setLeads(Array.isArray(result.leads) ? result.leads : []);
    } catch (error) {
      notify(error.message || 'Não foi possível carregar os leads.');
    }
  }
  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      setUser(result.user);
      await loadLeads(result.user.email);
      setScreen('dashboard');
  ...
  async function saveLead(event) {
    ...
    setLoading(true);
    try {
      await api('createLead', { lead: { ...data, origem: 'CRM Comercial' }, vendedor: user?.email || '' });
      form.reset();
      await loadLeads(user?.email);
      notify('Lead salvo com sucesso!');
      setScreen('leads');
    } catch (error) {
      notify(error.message || 'Não foi possível salvar o lead.');
    } finally {
      setLoading(false);
    }
  }
